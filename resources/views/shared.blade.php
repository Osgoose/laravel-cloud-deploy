{{-- resources/views/shared-lists/index.blade.php --}}
<x-layouts.app :title="__('LISTOPOLIS | COMPARTIDAS')">
  <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
    <div class="relative h-full flex-1 overflow-hidden rounded-xl border border-neutral-200 dark:border-neutral-700">

      {{-- Contenedor temable del shopping-app: SINCRONIZADO con Flux (igual que dashboard) --}}
      <div id="shopping-app"
      ...
      #shopping-app .s-category-btn{ flex:0 0 auto; }
        ...
        <div class="s-view @if(request('list')) s-hidden @endif">
          <header class="s-app-header">
            <h1 class="s-app-title">Listas compartidas conmigo</h1>
          </header>
          ...
          <div class="s-lists-grid" role="list">
            @forelse($shares as $share)
              @php
                $list = $share->list;
                $total = $list?->products?->sum('quantity') ?? 0;
              @endphp
              @if($list)
                <a class="s-list-card" role="listitem" href="{{ route('shared.lists', ['list' => $list->id]) }}">
            ...
            @empty
              <div class="s-empty-state" id="s-empty-state">
                ...
                <h3>No hay listas compartidas contigo</h3>
                <p>Cuando te compartan una lista a tu correo, aparecerá aquí</p>
              </div>
            @endforelse
          </div>

          @if(method_exists($shares, 'links'))
            <div class="mt-4 px-5">
              {{ $shares->links() }}
            </div>
          @endif
        ...
        {{-- Vista detalle (igual que dashboard pero el botón Volver regresa a shared.lists) --}}
        <div class="s-view @unless(request('list')) s-hidden @endunless">
          @if($current)
            <header class="s-app-header">
              <div class="s-header-nav">
                <a href="{{ route('shared.lists') }}" class="s-btn s-btn--outline" aria-label="Volver">
                  <span aria-hidden="true">←</span>&nbsp; Volver
                </a>
                <div class="s-header-actions">
                </div>
                <form action="{{ route('lists.unshareMe', $list->id) }}" method="post" class="inline-block" onsubmit="event.stopPropagation();">
  @csrf
  <button type="submit" class="s-btn s-btn--danger" style="margin-top:8px;">Quitar lista</button>
</form>
            ...
            {{-- Listado de productos y controles (exactos a dashboard) --}}
        ...
        {{-- Modales (usa los mismos del dashboard) --}}
        @if($current)
          ...
          {{-- Si quieres permitir renombrar/eliminar solo al dueño, reutiliza los modales y protégelos con @can --}}
        @endif
